...
class DynamixelClient:
    ...
    def read_pos_vel_cur(self, retries: int = 1, retry_interval: float = 0.0) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Returns the current positions and velocities."""
        return self._pos_vel_cur_reader.read(retries=retries, retry_interval=retry_interval)
    def read_pos_vel(self, retries: int = 1, retry_interval: float = 0.0) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Returns the current positions and velocities."""
        return self._pos_vel_reader.read(retries=retries, retry_interval=retry_interval)
    def read_pos(self, retries: int = 1, retry_interval: float = 0.0) -> np.ndarray:
        """Returns the current positions and velocities."""
        return self._pos_reader.read(retries=retries, retry_interval=retry_interval)
    def read_vel(self, retries: int = 1, retry_interval: float = 0.0) -> np.ndarray:
        """Returns the current positions and velocities."""
        return self._vel_reader.read(retries=retries, retry_interval=retry_interval)
    def read_cur(self, retries: int = 1, retry_interval: float = 0.0) -> np.ndarray:
        """Returns the current positions and velocities."""
        return self._cur_reader.read(retries=retries, retry_interval=retry_interval)
...
class DynamixelReader:
    ...
    def read(self, retries: int = 1, retry_interval: float = 0.0):
        ...
        while not success and retries >= 0:
            ...
            success = self.client.handle_packet_result(
                comm_result, context='read', log_errors=False)
            if not success and retries > 0 and retry_interval > 0.0:
                time.sleep(retry_interval)
            retries -= 1

        # If we failed, send a copy of the previous data.
        if not success:
            self.client.handle_packet_result(last_comm_result, context='read', log_errors=True)
